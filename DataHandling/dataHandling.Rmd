...
Analysisという名前の解析用フォルダを自分のパソコンの好きな場所に作成し，その中に，Dataという名前のデータを入れるフォルダを作成します。Dataフォルダ内には，[このように](Data)，csvファイルのデータだけをいれます。今回は，逆転学習課題について，４名分の仮想データを用意してみました。sub01.csvからsub04.csvまで４つのcsvファイルがあるかと思います。このように，データをいれるフォルダを作成して，そこに生データのcsvファイルのみを配置します。なお，フォルダとファイルと変数名に日本語を使用するのは推奨しません。フォルダとファイルと変数名には，英語（日本語のローマ字化でけっこう）を使用してください。それでは，sub01.csvからsub04.csvまで４つのcsvファイルをダウンロードして，AnalysisフォルダのDataフォルダ内に保存しましょう。
