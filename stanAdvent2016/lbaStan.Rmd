...
rtdistsのrLBA関数を用いて，A=0.5, b=1, tau(ここではt0) = 0.5, v[1]=2, v[2]=1.5, s=1のときの反応時間と反応を500個生成しました。作ったデータは，dataに格納し，さらにstanDataにStanで使えるようにリスト形式で保存しています。
...
A=0.5, b=1, tau(ここではt0) = 0.5, v[1]=2, v[2]=1.5, s=1で，データを作ったので，おおよそパラメータリカバリーできているかな。
