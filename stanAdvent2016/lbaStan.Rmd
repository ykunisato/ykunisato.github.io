...
## おまけ：データ数を増やしてパラメータリカバリ

上述の500試行のデータでは，パラメータリカバリはまあまあの結果でした。そこで，試行数を10000試行に増やして検討してみました。10000試行も実験とかできないので，あくまでパラメータリカバリの検討のためのものです。


```{r, message=FALSE, warning=FALSE}
#データセットの作成
set.seed(1234)
data10000 <- rLBA(10000, A=0.5, b=1, t0 = 0.5, mean_v=c(2, 1.5), sd_v=c(1,1))
trialLength10000 = length(data10000$rt)
stanData10000 <- list(rt=data10000$rt,res=data10000$response,LENGTH=trialLength10000,NUM_CHOICES=2)
...
fit10000 <- sampling(lbaModel, 
            ...
            chains = hmcChains,
            thin =hmcThin)
extractFit10000 <- rstan::extract(fit10000)
```


A=0.5, b=1, tau(ここではt0) = 0.5, v[1]=2, v[2]=1.5, s=1でデータを作っています。500よりは，うまくできているかな？
```{r, message=FALSE, warning=FALSE}
posterior10000 <- as.matrix(fit10000)
kMap <- mapEstimate(extractFit10000$k)
AMap <- mapEstimate(extractFit10000$A)
tauMap <- mapEstimate(extractFit10000$tau)
v1Map <- mapEstimate(extractFit10000$v[,1])
v2Map <- mapEstimate(extractFit10000$v[,2])

print(fit10000,par = c("k", "A", "tau", "v[1]", "v[2]","s"))
# MAP推定値
print(paste("k=",kMap,",A=",AMap,",tau=",tauMap,",v1=",v1Map,",v2=",v2Map))

mcmc_areas(posterior10000, pars = c("k", "A", "tau", "v[1]", "v[2]"), prob = 0.95)
```
