...
parameters {
     real < lower = 0 > k[numberSubjects];
     vector < lower = 0 > [numberConditions] A [numberSubjects];
     real < lower = 0 > psi[numberSubjects];
     vector < lower = 0 > [numberChoices] v [numberSubjects, numberConditions];
     real < lower = 0 > k_sigma;
     vector < lower = 0 > [numberConditions] A_sigma;
     real < lower = 0 > psi_sigma;
     vector < lower = 0 > [numberChoices] v_sigma [numberConditions];
     real < lower = 0 > k_mu;
     vector < lower = 0 > [numberConditions] A_mu;
     real < lower = 0 > psi_mu;
...
model {
     ...
     for( j in 1 : numberConditions){
        A_sigma[j] ~ student_t(4, 0, 1)T[0, ];
        A_mu[j] ~ normal(.5, 1)T[0, ];
          for(n in 1 : numberChoices){
     ...
     for(i in 1 : numberSubjects){		
        ...
        psi[i] ~ normal(psi_mu, psi_sigma)T[0, ];
          for( j in 1 : numberConditions){
            A[i, j] ~ normal(A_mu[j], A_sigma[j])T[0, ];
              for(n in 1 : numberChoices){
    ...
    for(m in 1 : numberAllTrials){
      ...
      rt[m] ~ lba(response[m], k[subIndex], A[subIndex, condIndex], v[subIndex, condIndex], s, psi[subIndex]);
    }
}
...
generated quantities {
     ...
     for(i in 1 : numberSubjects){
          for(j in 1 : numberConditions){
               pred[i, j] = lba_rng(k[i], A[i, j], v[i, j], s, psi[i]);
          }
     }
    ...
    for(m in 1 : numberAllTrials){
          subIndex = id[m];
          condIndex = condition[m];
          log_lik[m] = lba_lpdf(rt[m] | response[m], k[subIndex], A[subIndex, condIndex], v[subIndex, condIndex], s, psi[subIndex]);
    }
